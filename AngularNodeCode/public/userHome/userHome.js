...
.controller('UserHomeCtrl',['$scope','CommonProp','$http','$location', function($scope,CommonProp,$http,$location) {
	...
	var getAllTask = function(){
		...
		$http({method: 'GET',cache: false, url: 'http://127.0.0.1:5000/article?where={"username":"'+user+'"}'}).
	...
	$scope.update = function(){
		...
		$http({method: 'PATCH',cache: false, url: 'http://127.0.0.1:5000/article/'+ $scope.edit.id,data: { name: $scope.edit.task }}).
		    success(function(data, status, headers, config) {
	...
	$scope.deleteTask = function(){
		...
		$http({method: 'DELETE',cache: false, url: 'http://127.0.0.1:5000/article/'+ $scope.deletion.id}).
		    success(function(data, status, headers, config) {
